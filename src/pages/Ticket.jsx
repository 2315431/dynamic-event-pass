...
const staticStyles = {
  ...
  backup: { marginTop: '30px', padding: '15px', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '10px', fontSize: '12px' },
...
function Ticket() {
  ...
  useEffect(() => {
    ...
    try {
      const payload = jwtDecode(ticketJWT);
  ...
  if (error) {
    return <div style={staticStyles.container}><div style={staticStyles.message}>❌ {error}</div></div>;
  }
  if (!ticketData) {
    return <div style={staticStyles.container}><div style={staticStyles.message}>Loading ticket...</div></div>;
  }
    ...
    <div style={staticStyles.container}>
      <div style={dynamicTicketStyle}>
        ...
        <div style={staticStyles.details}>
          ...
          <div style={staticStyles.detailRow}><span>Category:</span><span>{ticketData.category}</span></div>
          <div style={staticStyles.detailRow}><span>Seat:</span><span>{ticketData.seatInfo}</span></div>
          <div style={staticStyles.detailRow}><span>Valid Until:</span><span>{new Date(ticketData.validTo).toLocaleDateString()}</span></div>
        </div>
        <div style={staticStyles.backup}><strong>Works Offline!</strong><br/>This code updates automatically without internet.</div>
      </div>
    </div>
  );
}
